...
void RelayPlugin::SendMessageToRoom(StrAndGuidAndRoom **strAndGuidSender, BitStream* message)
{
	...
	for (unsigned int i=0; i < chatRooms.Size(); i++)
	{
		if (chatRooms[i]->roomName==(*strAndGuidSender)->currentRoom)
		{
			...
			RP_Group *room = chatRooms[i];
			for (unsigned int ii=0; ii < room->usersInRoom.Size(); ii++)
			{
				if (room->usersInRoom[ii].guid!=(*strAndGuidSender)->guid)
					SendUnified(&bsOut, HIGH_PRIORITY, RELIABLE_ORDERED, 0, room->usersInRoom[ii].guid, false);
			}
...
void RelayPlugin::OnGroupMessageFromClient(Packet *packet)
{
	...
	unsigned char cIn;
	bsIn.Read(cIn);
	// PacketPriority priority = (PacketPriority) cIn;
	bsIn.Read(cIn);
	// PacketReliability reliability = (PacketReliability) cIn;
	bsIn.Read(orderingChannel);
	BitStream bsData;
	bsIn.Read(&bsData);
